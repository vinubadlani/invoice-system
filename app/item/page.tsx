...
import { queryBuilder, insertData, updateData, deleteData } from "@/lib/supabase"
...
export default function ItemMaster() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!editingItem) {
        const existingItems = await queryBuilder("items", "id,name,code", {
      ...
      if (editingItem) {
        const { error } = await updateData("items", editingItem.id, cleanFormData)
        ...
        const { error } = await insertData("items", { ...cleanFormData, business_id: businessId })
  ...
  const handleDelete = async (item: Item) => {
    ...
    try {
      const { error } = await deleteData("items", item.id)
