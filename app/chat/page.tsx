...
export default function ChatPage() {
  ...
  const loadPartiesAndItems = async () => {
    ...
    try {
      ...
      // Load parties and items in parallel
      const [partiesResponse, itemsResponse] = await Promise.all([
        client.from('parties').select('*').eq('business_id', selectedBusiness.id),
        client.from('items').select('*').eq('business_id', selectedBusiness.id)
      ])
      ...
      toast({
        title: 'Error',
        description: 'Failed to load parties and items data',
        variant: 'destructive'
      })
    } finally {
  ...
  const handleSaleCommand = async (input: string): Promise<Omit<Message, 'id' | 'timestamp'>> => {
    try {
      // If we have parties and items available, show interactive form
      if (parties.length > 0 || items.length > 0) {
        setShowSaleForm(true)
        return {
          type: 'bot',
          content: `🛒 **Create Sale Entry**\n\nI found ${parties.length} customers and ${items.length} items in your database.\n\nPlease use the form below to create your sale:`,
          actions: [
            {
              label: 'Show Selection Form',
              action: () => setShowSaleForm(true)
            }
          ],
          suggestions: ['Use form below', 'Add new customer', 'Add new item']
        }
      }
      
      // Fallback to text parsing if no data available
      return await createSaleFromText(input)
    } catch (error: any) {
      return {
        type: 'bot',
  ...
  const handleSaleFormSubmit = async () => {
    try {
      if (!saleForm.partyId || !saleForm.itemId) {
        toast({
          title: 'Error',
          description: 'Please select both customer and item',
          variant: 'destructive'
        })
        return
      }

      const selectedParty = parties.find(p => p.id === saleForm.partyId)
      const selectedItem = items.find(i => i.id === saleForm.itemId)
      
      if (!selectedParty || !selectedItem) {
        toast({
          title: 'Error',
          description: 'Selected customer or item not found',
          variant: 'destructive'
        })
        return
      }

      const price = saleForm.price || selectedItem.sales_price || 0
      const quantity = saleForm.quantity || 1
      ...
      const saleData = {
        ...
        address: selectedParty.address || 'Address not specified',
      ...
      addMessage({
        type: 'bot',
        content: `✅ **Sale Saved Successfully!**\n\n**Invoice No:** ${saleData.invoice_no}\n**Customer:** ${selectedParty.name}\n**Item:** ${selectedItem.name}\n**Quantity:** ${quantity}\n**Total:** ₹${total.toLocaleString()}\n\nThe sale has been saved to your database.`,
        suggestions: ['Create another sale', 'View sales report', 'Add a purchase']
      })
      ...
      toast({
        title: 'Error',
        description: `Failed to save sale: ${error.message}`,
        variant: 'destructive'
      })
  ...
  const createSaleFromText = async (input: string): Promise<Omit<Message, 'id' | 'timestamp'>> => {
    try {
        ...
        <div className="max-w-6xl mx-auto p-2 sm:p-6">
          ...
          <Card className="h-[70vh] sm:h-[600px] flex flex-col shadow-xl border-0">
            ...
            <CardContent className="flex-1 flex flex-col p-0">
              ...
              {/* Sale Form - Mobile Responsive */}
              {showSaleForm && (
                <div className="p-3 sm:p-6 bg-gradient-to-r from-blue-50 to-purple-50 border-t border-slate-200">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                    <div>
                      <Label htmlFor="party-select" className="text-xs sm:text-sm">Customer *</Label>
                      <Select value={saleForm.partyId} onValueChange={(value) => setSaleForm(prev => ({ ...prev, partyId: value }))}>
                        <SelectTrigger className="text-xs sm:text-sm">
                          <SelectValue placeholder="Select customer" />
                        </SelectTrigger>
                        <SelectContent>
                          {parties.map((party) => (
                            <SelectItem key={party.id} value={party.id} className="text-xs sm:text-sm">
                    ...
                    <div>
                      <Label htmlFor="item-select" className="text-xs sm:text-sm">Item *</Label>
                      <Select 
                        ...
                        onValueChange={(value) => {
                          ...
                          setSaleForm(prev => ({ 
                            ...prev, 
                            itemId: value,
                            price: selectedItem?.sales_price || 0
                          }))
                        }}
                      >
                        <SelectTrigger className="text-xs sm:text-sm">
                          ...
                          {items.map((item) => (
                            <SelectItem key={item.id} value={item.id} className="text-xs sm:text-sm">
                              <div className="flex flex-col">
                                ...
                                <span className="text-xs text-gray-500">₹{item.sales_price?.toLocaleString() || 0} per {item.unit || 'unit'}</span>
                              </div>
                            </SelectItem>
                          ))}
                        </SelectContent>
                    ...
                    <div>
                      <Label htmlFor="price" className="text-xs sm:text-sm">Price per unit</Label>
                      <Input
                        id="price"
                        type="number"
                        min="0"
                        value={saleForm.price}
                        onChange={(e) => setSaleForm(prev => ({ ...prev, price: parseFloat(e.target.value) || 0 }))}
                        className="text-xs sm:text-sm"
                  ...
                  {/* Sale Summary */}
                  {saleForm.partyId && saleForm.itemId && (
                    <div className="mt-4 p-3 bg-white rounded-lg border border-slate-200">
                      <div className="text-xs sm:text-sm space-y-1">
                        <div className="flex justify-between">
                          <span>Subtotal:</span>
                          <span>₹{(saleForm.quantity * saleForm.price).toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between">
                          <span>GST (18%):</span>
                          <span>₹{((saleForm.quantity * saleForm.price) * 0.18).toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between font-semibold border-t pt-1">
                          <span>Total:</span>
                          <span>₹{((saleForm.quantity * saleForm.price) * 1.18).toLocaleString()}</span>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="mt-4 flex gap-2">
                    <Button 
                      onClick={handleSaleFormSubmit}
                      disabled={!saleForm.partyId || !saleForm.itemId || saleForm.quantity < 1}
                    ...
                    <Button 
                      variant="outline" 
                      onClick={() => {
                        setSaleForm({ partyId: '', itemId: '', quantity: 1, price: 0 })
                        setShowSaleForm(false)
                      }}
                      className="text-xs sm:text-sm"
                    >
