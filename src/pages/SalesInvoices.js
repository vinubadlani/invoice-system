...
const SalesInvoices = () => {
  ...
  const stats = {
    ...
    totalRevenue: invoices.reduce((sum, inv) => sum + Number(inv.net_total), 0),
    totalReceived: invoices.reduce((sum, inv) => sum + Number(inv.payment_received), 0),
    totalPending: invoices.reduce((sum, inv) => sum + Number(inv.balance_due), 0),
    paid: invoices.filter(inv => inv.status === 'paid').length,
    pending: invoices.filter(inv => inv.status === 'sent' || inv.status === 'partial').length,
