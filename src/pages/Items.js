...
const Items = () => {
  ...
  const filteredItems = Array.isArray(items)
    ? items.filter(item =>
        ...
        typeof item.hsn_code === 'string' && item.hsn_code.includes(searchTerm)
      )
    : [];

  const totalInventoryValue = Array.isArray(items)
    ? items.reduce((sum, item) => sum + ((item?.opening_stock ?? 0) * (item?.sales_price ?? 0)), 0)
    : 0;

  const totalStock = Array.isArray(items)
    ? items.reduce((sum, item) => sum + (item?.opening_stock ?? 0), 0)
    : 0;
