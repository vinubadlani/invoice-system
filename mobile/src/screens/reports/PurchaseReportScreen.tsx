...
import { computeSalesSummary, topCustomers, topProducts } from '../../utils/analytics';
import { formatCurrency } from '../../utils/format';

type Props = NativeStackScreenProps<ReportStackParamList, 'PurchaseReport'>;

export function PurchaseReportScreen({}: Props) {
  const { data: invoices, loading, error, reload } = useBusinessData((businessId) => api.invoice.getAll(businessId, 'purchase'));

  const summary = useMemo(() => computeSalesSummary(invoices ?? []), [invoices]);
  const suppliers = useMemo(() => topCustomers(invoices ?? [], 10), [invoices]);
    ...
    <Screen edges={['left', 'right']}>
      <ScrollView contentContainerStyle={styles.content}>
        <Card style={styles.summaryCard}>
          <SummaryRow label="Total Purchases" value={formatCurrency(summary.totalSales)} bold />
          <SummaryRow label="Bills" value={String(summary.invoiceCount)} />
          <SummaryRow label="Average Bill" value={formatCurrency(summary.averageInvoiceValue)} />
          <SummaryRow label="Paid" value={String(summary.paidCount)} />
          <SummaryRow label="Outstanding (Payable)" value={formatCurrency(summary.outstanding)} />
        </Card>

        <Text variant="subtitle" weight="semibold" style={styles.sectionTitle}>Purchases by Supplier</Text>
        <Card style={styles.tableCard}>
          {suppliers.length === 0 ? <Text variant="body" tone="muted">No data</Text> : suppliers.map((c) => (
        ...
        <Text variant="subtitle" weight="semibold" style={styles.sectionTitle}>Purchases by Product</Text>
        <Card style={styles.tableCard}>
