...
type Props = NativeStackScreenProps<SalesStackParamList, 'SalesDetail'>;

export function SalesDetailScreen({ route, navigation }: Props) {
  ...
  useLayoutEffect(() => {
    navigation.setOptions({ title: invoice?.invoice_no ?? 'Sale' });
  }, [invoice, navigation]);

  const handleDelete = async () => {
    if (!invoice) return;
    const ok = await confirm({ title: 'Delete invoice?', message: `This permanently deletes invoice ${invoice.invoice_no}.`, confirmLabel: 'Delete', destructive: true });
    if (!ok) return;
    try {
    ...
    <Screen edges={['left', 'right']}>
      <ScrollView contentContainerStyle={styles.content}>
        <InvoiceDetailHero
          ...
          actions={[
            { label: 'Edit Sale', icon: 'create-outline', onPress: () => navigation.navigate('SalesEntry', { invoiceId: invoice.id }) },
        ...
        <Text variant="subtitle" weight="semibold" style={styles.sectionTitle}>Services</Text>
        ...
        <Card style={styles.totalsCard}>
          ...
          <TotalRow label="Payment Received" value={formatCurrency(invoice.payment_received)} />
          <TotalRow
            label="Balance Due"
