...
type Props = NativeStackScreenProps<SalesStackParamList, 'SalesEntry'>;

export function SalesEntryScreen({ route, navigation }: Props) {
  ...
  useEffect(() => {
    navigation.setOptions({ title: isEdit ? 'Edit Sale' : 'New Sale' });
  }, [isEdit, navigation]);
    ...
    (async () => {
      try {
        const [partyList, itemList, invoiceList] = await Promise.all([
          ...
          api.invoice.getAll(selectedBusiness.id, 'sales'),
        ]);
        ...
        if (invoiceId) {
          ...
          setInvoiceNo(generateInvoiceNumber('sales'));
        }
      } catch (e: any) {
        toast.error(e.message ?? 'Could not load sale data');
      } finally {
        setLoading(false);
      }
    })();
  ...
  const handleSave = async () => {
    ...
    if (dup) {
      setInvoiceNoError(`Invoice number "${invoiceNo}" already exists`);
      return;
    }
    ...
    try {
      ...
      toast.error(e.message ?? 'Could not save sale');
    } finally {
      setSaving(false);
    }
  };
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.row2}>
          <View style={styles.flex1}>
            <Input label="Invoice No." required value={invoiceNo} onChangeText={setInvoiceNo} error={invoiceNoError ?? undefined} />
          </View>
          <View style={styles.flex1}>
        ...
        <View style={[styles.gstToggleCard, { borderColor: theme.colors.border, backgroundColor: theme.colors.card }]}>
          ...
          {isGst ? (
            <View style={styles.taxModeRow}>
              <TaxModeChip label="CGST + SGST (Intra-state)" active={taxMode === 'intra'} onPress={() => setTaxMode('intra')} />
              <TaxModeChip label="IGST (Inter-state)" active={taxMode === 'inter'} onPress={() => setTaxMode('inter')} />
            </View>
          ) : null}
        </View>
        ...
        <Input label="Payment Received (₹)" value={paymentReceived} onChangeText={setPaymentReceived} keyboardType="decimal-pad" />

        <InvoiceTotalsSummary totals={totals} gstEnabled={isGst} taxMode={taxMode} />

        <Button label={isEdit ? 'Save Changes' : 'Save Sale'} onPress={handleSave} loading={saving} fullWidth style={styles.submit} />
      </ScrollView>
    </Screen>
  );
}

function TaxModeChip({ label, active, onPress }: { label: string; active: boolean; onPress: () => void }) {
  const theme = useTheme();
  return (
    <Button
      label={label}
