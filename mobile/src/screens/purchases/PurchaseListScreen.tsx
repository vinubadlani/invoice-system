    ...
    <Screen edges={['left', 'right']}>
      {!loading ? (
        <View style={styles.summaryRow}>
          <SummaryChip label="Bills" value={String(summary.count)} />
          <SummaryChip label="Total" value={formatCurrency(summary.total)} />
          <SummaryChip label="Payable" value={formatCurrency(summary.outstanding)} tone="critical" />
        </View>
      ) : null}
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <EmptyState
              icon="cart-outline"
              title={query || statusFilter !== 'all' ? 'No matching bills' : 'No purchases yet'}
              description={query || statusFilter !== 'all' ? 'Try a different search or filter.' : 'Record your first purchase bill.'}
              actionLabel={!query && statusFilter === 'all' ? 'New Purchase' : undefined}
              onAction={!query && statusFilter === 'all' ? () => navigation.navigate('PurchaseEntry', undefined) : undefined}
            />
          }
          renderItem={({ item }: { item: Invoice }) => (
            <Pressable onPress={() => navigation.navigate('PurchaseDetail', { invoiceId: item.id })}>
