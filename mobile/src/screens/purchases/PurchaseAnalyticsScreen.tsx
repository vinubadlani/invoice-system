...
type Props = NativeStackScreenProps<PurchaseStackParamList, 'PurchaseAnalytics'>;
...
export function PurchaseAnalyticsScreen({}: Props) {
  const theme = useTheme();
  const [preset, setPreset] = useState<DateRangePreset>('month');

  const { data: invoices, loading, refreshing, error, refresh, reload } = useBusinessData(
    (businessId) => api.invoice.getAll(businessId, 'purchase'),
  );
  ...
  const suppliers = useMemo(() => topCustomers(filtered), [filtered]);
      ...
      >
        ...
        {loading ? (
          ...
          <View style={styles.statsGrid}>
            <StatTile label="Total Purchases" value={formatCurrencyCompact(summary.totalSales)} icon="cart-outline" />
            <StatTile label="Bills" value={String(summary.invoiceCount)} icon="document-text-outline" />
            <StatTile label="Avg. Bill" value={formatCurrencyCompact(summary.averageInvoiceValue)} icon="calculator-outline" />
            <StatTile label="Payable" value={formatCurrencyCompact(summary.outstanding)} icon="alert-circle-outline" deltaTone={summary.outstanding > 0 ? 'critical' : 'neutral'} />
          </View>
        )}

        <Card style={styles.section}>
          <Text variant="subtitle" weight="semibold" style={styles.sectionTitle}>Purchase Trend (6 months)</Text>
        ...
        <Card style={styles.section}>
          ...
          <View style={styles.momRow}>
            ...
            <View>
              ...
              <Text variant="subtitle" weight="bold">{mom.deltaPercent >= 0 ? '+' : ''}{formatPercent(mom.deltaPercent)}</Text>
            </View>
          </View>
        </Card>

        <Text variant="subtitle" weight="semibold" style={styles.sectionTitle}>Top Suppliers</Text>
        {suppliers.length === 0 ? <Text variant="body" tone="muted">No purchases in this period</Text> : suppliers.map((c, i) => (
          <RankRow key={c.name} rank={i + 1} name={c.name} value={formatCurrency(c.value)} sub={`${c.count} bill${c.count === 1 ? '' : 's'}`} />
        ))}

        <Text variant="subtitle" weight="semibold" style={styles.sectionTitle}>Top Purchased Products</Text>
        {products.length === 0 ? <Text variant="body" tone="muted">No purchases in this period</Text> : products.map((p, i) => (
          <RankRow key={p.name} rank={i + 1} name={p.name} value={formatCurrency(p.value)} sub={`${p.count} units`} />
        ))}
      </ScrollView>
    </Screen>
  );
