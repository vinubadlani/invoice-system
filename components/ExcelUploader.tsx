...
PUR001,2025-01-15,ABC Suppliers,Maharashtra,123 Supplier St Mumbai,27ABCDE1234F1Z5,Sample Product,1234,10,100,18,180,1180,500,Cash,Sample purchase entry`
  ...
  const addNewRow = () => {
    ...
    setData(Array.isArray(data) ? [...data, newRow] : [newRow])
  }

  const deleteRow = (rowId: string) => {
    setData(Array.isArray(data) ? data.filter(row => row.id !== rowId) : [])
  }
  ...
  const submitData = async () => {
    ...
    const validRows = Array.isArray(data) ? data.filter(row => row.status !== 'error') : [];
